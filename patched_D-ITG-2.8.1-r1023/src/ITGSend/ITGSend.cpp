...
void *flowSender(void *para)
{
	...
	//akhila
	//typedef std::tuple<long int, long int> tx_time_t;
	//std::map<unsigned int, tx_time_t> seqNum_txTstamp_map;
        //std::map<unsigned int, tx_time_t>::iterator map_it;
	//long int tmp_time_sec = 0;
	//long int tmp_time_Usec = 0;
        long int time1Usec = 0;
	...
	do {
		...
		do {
			...
			net_size = htonl(size);
			//akhila
//			printf("seqNum: %d stored Tx time values in variables\n",seqNum);
			...
			if ((l4Proto == L4_PROTO_ICMP) && (meter == METER_OWDM)) {
				...
				//akhila
//				printf("seqNum %d socket send\n",seqNum);
				//akhila 
				// store the seq num and the tx time stamp in a map 
				// to retrieve later when I receive this back from ITGRecv.  
			}
#else
			} else if (l4Proto != L4_PROTO_SCTP) {
				sockchk = send(sock, (char *) payload, size, 0);
			...
			if (sockchk < 0 && !termsock) {
				...
				if ((logging) && (meter == METER_RTTM)) {
					// akhila
					// pkt has been sent
                        		//seqNum_txTstamp_map.insert(pair<long int, long int>(seqNum, time));
//					printf("seqNum %d saved in map with tx tstamp: %ld, %ld \n",seqNum,time,timeUsec);
                        		//seqNum_txTstamp_map[seqNum] = std::make_tuple (time,timeUsec);
					}
			...
			UPDATE_TICKER(&Ticker, _tend, _tprec, _tstart, first_update);
                        // akhila
//                        printf("seqNum: %d updateTicker. Update the lastTime value\n",seqNum);
			...
			while (!exitflag) {
				...
				if (meter == METER_RTTM) {
					...
					if (FD_ISSET(sock, &read_set)) {
						...
						//akhila
//						printf("seqNum: %d obtain recv timestamp becasue bit for recv is set.\n",seqNum);
						...
						//akhila
						time1Usec = ntohl((*(uint32_t *) ptrTimeUsec));
						//akhila
						//time2 = RcvTime.tv_sec % 86400L;
						time2 = RcvTime.tv_sec;
						recvPkts++;

						//akhila
//						printf("seqNum: %d sock recv\n",ntohl(*(uint32_t *) ptrSeqNum));
						
						
						if ((logging) && (size_r > 0) && (meter == METER_RTTM)) {
/*
							//akhila
                                                	map_it = seqNum_txTstamp_map.find(ntohl(*(uint32_t *) ptrSeqNum));
							if(map_it == seqNum_txTstamp_map.end()){
  //      							printf("ERROR: recv RTTM response for seqNum %d not present in map\n",ntohl(*(uint32_t *) ptrSeqNum)); 
								}
    							else{
//								printf("recv RTTM response for seqNum %d. current map size: %ld\n",ntohl(*(uint32_t *) ptrSeqNum),seqNum_txTstamp_map.size());
        							std::tie(tmp_time_sec, tmp_time_Usec) = map_it->second;
								// remove this entry now
								seqNum_txTstamp_map.erase (map_it);

								// remove any entry that is really old 
								map_it = seqNum_txTstamp_map.begin();
								while (map_it != seqNum_txTstamp_map.end()){
									if(seqNum - map_it->first > 1000){
//										printf("Have to erase pkt with seqNum %d because the current running seqNum is %d\n",map_it->first, seqNum);
										//fflush(stdout);
										seqNum_txTstamp_map.erase (map_it);
										map_it++;
										}
...
*/
							...
							if (logremoto == 0) {
								...
								writeInBufferStandard(&infos[count], *(uint32_t *) payload,
                                                                                *(uint32_t *) ptrSeqNum, HelpSrcAddress,
                                                                                HelpDstAddress, tmpPort_SrcPort, tmpPort_DstPort,
                                                                                time1, time2, time1Usec, RcvTime.tv_usec, size_r);

								// akhila
//								printf("seqNum: %d Inside if ((logging) && (size_r > 0) && (meter == METER_RTTM)) \n", seqNum);
							} else {
			...
			seqNum++;
			//akhila
//			printf("seqNum: %d seqNum incremented\n",seqNum);
	...
	if (meter == METER_RTTM) {
		//akhila
//		printf("seqNum %d inside the if (meter == METER_RTTM) { section outside the do while loop\n",seqNum);
		bool flag = true;
		while (flag && (seqNum-1) > recvPkts) {
			...
			if (select(FD_SETSIZE, &read_set, NULL, NULL, &start_time) < 0) {
				...
				if (FD_ISSET(sock, &read_set)) {
					...
					recvPkts++;
//					printf("seqNum: %d after receiving response in section outside the do while loop\n",seqNum);
...
//							printf("recv RTTM response for seqNum %d\n",ntohl(*(uint32_t *) ptrSeqNum));
                                                	std::tie(tmp_time_sec, tmp_time_Usec) = map_it->second;
                                                        // remove this entry now
                                                        seqNum_txTstamp_map.erase (map_it);
                                                	}
*/
						if (logremoto == 0) {
							...
							writeInBufferStandard(&infos[count], *(uint32_t *) payload,
                                                                        *(uint32_t *) ptrSeqNum, HelpSrcAddress, HelpDstAddress,
                                                                        tmpPort_SrcPort, tmpPort_DstPort, time1, time2, time1Usec,
                                                                        RcvTime.tv_usec, size_r);
						} else {
