...
void writeline(int filetype)
{
	if (filetype == 0) {
		//akhila
		unsigned long int tx_epochtime_usec;
		unsigned long int rx_epochtime_usec;

		//fprintf(outtxt, "Flow>%04u ", (*infos).flowId);
		//fprintf(outtxt, "Seq>%7u ", (*infos).seqNum);
		//akhila
		fprintf(outtxt, "Flow %u ", (*infos).flowId);
		fprintf(outtxt, "Seq %u ", (*infos).seqNum);
#ifdef V23
		//fprintf (outtxt, "Src>%16s/%hu ",(*infos).srcAddr,(*infos).srcPort);
		//fprintf (outtxt, "Dest>%16s/%hu ",(*infos).destAddr,(*infos).destPort);
		//akhila
		fprintf (outtxt, "Src %s/%hu ",(*infos).srcAddr,(*infos).srcPort);
		fprintf (outtxt, "Dest %s/%hu ",(*infos).destAddr,(*infos).destPort);
#else
		...
		//akhila
		tx_epochtime_usec = ((uint64_t)((*infos).txTime1) * (uint64_t)3600000000) + ((uint64_t)((*infos).txTime2) * (uint64_t)60000000) + (uint64_t)floor( (*infos).txTime3 * (double)1000000.0);
		rx_epochtime_usec = ((uint64_t)((*infos).rxTime1) * (uint64_t)3600000000) + ((uint64_t)((*infos).rxTime2) * (uint64_t)60000000) + (uint64_t)floor( (*infos).rxTime3 * (double)1000000.0);
		fprintf(outtxt, "txTime %lu ", tx_epochtime_usec);
		fprintf(outtxt, "rxTime %lu ", rx_epochtime_usec);
		fprintf(outtxt, "Size %u\n", (*infos).size);
	} else {
